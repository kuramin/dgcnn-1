...
def get_edge_feature(point_cloud, nn_idx, k=20):
  ...
  """
    ...
    idx_ = tf.range(batch_size) * num_points  # will be [0, batch_size, 2*batch_size, 3*batch_size, ... ]
    idx_ = tf.reshape(idx_, [batch_size, 1, 1])

    point_cloud_flat = tf.reshape(point_cloud, [-1, num_dims])  # -1 is a synonim of (batch_size * num_points) here, so points from all clouds of batch will be lined up
